...
function TableComponent(props) {
    ...
    const fetchLeaves=()=>{ 
      fetch('https://react-authentication-4e6e7-default-rtdb.firebaseio.com/SampleDB.json').then(data=>{
        return data.json();
      }).then(data=>{
        let newdata=[];
    ...
    return (
        ...
        {props.details.length===0?"No record to display":
         searchResult.map(item=>{
            ...
            <><Button variant="success" size="sm" onClick={()=>{OnAccept(item,'Approved')}} id="acc">APPROVE</Button> 
            ...
            <td>{<Button variant="danger" size="sm" onClick={()=>{OnDelete(item)}} >DELETE</Button>}</td>
